  ...
  private
    ...
    def quoter_profile_params
      if params[:quoter_profile][:user_attributes][:password].blank? and params[:quoter_profile][:user_attributes][:password_confirmation].blank?
        params.require(:quoter_profile).permit(:name, :user_attributes => [:username, :email])
      else
        params.require(:quoter_profile).permit(:name, :user_attributes => [:username, :email, :password, :password_confirmation])
      end
    end
end
