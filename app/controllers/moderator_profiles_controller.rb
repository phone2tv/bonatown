  ...
  private
    ...
    def moderator_profile_params
      if params[:moderator_profile][:user_attributes][:password].blank? and params[:moderator_profile][:user_attributes][:password_confirmation].blank?
        params[:moderator_profile][:user_attributes].delete :password
        params[:moderator_profile][:user_attributes].delete :password_confirmation
      end
      params.require(:moderator_profile).permit(:name, :user_attributes => [:username, :email, :password, :password_confirmation, :name, :card_type, :card_no, :birthday, :gender, :english_name, :mobile, :telephone, :province, :city, :district, :location])
    end
end
