  ...
  private
    ...
    def customer_profile_params
      if params[:customer_profile][:user_attributes][:password].blank? and params[:customer_profile][:user_attributes][:password_confirmation].blank?
        params.require(:customer_profile).permit(:name, :enterprise_name, :province_code, :city_code, :district_code, :location, :business_license, :park_profile_id, :user_attributes => [:username, :email])
      else
        params.require(:customer_profile).permit(:name, :enterprise_name, :province_code, :city_code, :district_code, :location, :business_license, :park_profile_id, :user_attributes => [:username, :email, :password, :password_confirmation])
      end
    end
end
