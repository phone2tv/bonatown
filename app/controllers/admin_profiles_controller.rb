  ...
  private
    ...
    def admin_profile_params
      if params[:admin_profile][:user_attributes][:password].blank? and params[:admin_profile][:user_attributes][:password_confirmation].blank?
        params[:admin_profile][:user_attributes].delete :password
        params[:admin_profile][:user_attributes].delete :password_confirmation
      end
      params.require(:admin_profile).permit(:name, :user_attributes => [:username, :email, :password, :password_confirmation, :name, :card_type, :card_no, :birthday, :gender, :english_name, :mobile, :telephone, :province, :city, :district, :location])
    end
end
